...
from persistent_currency import get_balance, set_balance
...
class TheSimpsonsTappedOutLocalServer:
  ...
  def protocurrency(self, land_id: str):
    ...
    # Load (and initialize) the stored donut balance for this land_id.
    balance = get_balance(land_id)

    currency = PurchaseData_pb2.CurrencyData()
    currency.id = land_id
    currency.vcTotalPurchased = 0
    currency.vcTotalAwarded = 0
    currency.vcBalance = balance                    # number of donuts (persisted)
    now_ms = int(round(time.time() * 1000))
    currency.createdAt = now_ms
    currency.updatedAt = now_ms
  ...
  def extraLandUpdate(self, land_id: str):
    ...
    # Parse the incoming ExtraLandMessage
    req = LandData_pb2.ExtraLandMessage()
    try:
      req.ParseFromString(request.data)
    except Exception as e:
      self.log_error(f"Failed to parse ExtraLandMessage: {e}")
      abort(400)
    ...
    if total_delta != 0:
      # Load current persisted balance and apply the delta(s)
      try:
        current = get_balance(land_id)
      except Exception as e:
        self.log_error(f"Failed to read balance for {land_id}: {e}")
        current = 0
      new_total = current + total_delta
      if new_total < 0:
        new_total = 0
      try:
        set_balance(land_id, new_total)
        self.log_debug(f"Updated donuts for {land_id}: {current} -> {new_total}")
      except Exception as e:
        self.log_error(f"Failed to persist balance for {land_id}: {e}")

    # Respond with an ExtraLandResponse protobuf (same as before)
